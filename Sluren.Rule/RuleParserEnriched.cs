...
namespace Sluren.Rule {
	public class RuleParserEnriched : RuleVisitor<RuleParsingResult> {
		protected object dataToEvaluateRuleOn;
		public RuleParserEnriched(object ruleInput) {
			dataToEvaluateRuleOn = ruleInput;
			if (dataToEvaluateRuleOn == null)
		...
		public override RuleParsingResult Visit(HardValueBasedRule rule) {
			...
			return new RuleParsingResult(
				new BooleanCondition((bool)operand.GetMethod(rule.Comparator).Invoke(operandObject, paramContainer))
				, objectForOperand.ToString()
				, paramContainer.ToString()
				, ""
			);
		}
		public override RuleParsingResult Visit(PropertyFieldBasedRule rule) {
			...
			if (rule.Right.Contains("and")) {
				...
				rightOperand = paramContainer[0].ToString() + " and " + paramContainer[1].ToString();
			}
			else if (rule.Right.Contains(",")) {
				string[] strArr = rule.Right.Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries);
				if (strArr.Length < 1)
				...
				foreach (var item in paramHost) {
					rightOperand += item.ToString() + ", ";
				}
			}
			else {
				paramContainer = new object[] { GetFinalObjectFromExpression(dataToEvaluateRuleOn, rule.Right.Trim()) };
				rightOperand = paramContainer[0].ToString();
			}
		...
		public override RuleParsingResult Visit(PropertyFieldToHardValueRule rule) {
			throw new NotImplementedException();
		}
		public override RuleParsingResult Visit(TypeBasedRule rule) {
			...
			return new RuleParsingResult(
				new BooleanCondition(results.Distinct().Count() > 1 ? false : results.ElementAt(0))
				, targetType.ToString()
				, rule.Right.ToString()
				, ""
			);
		}
		public override RuleParsingResult Visit(CompositeRule rule) {
			CompositeCondition synthesizedCondition = new CompositeCondition();

			foreach (var ruleItem in rule.rules) {
				RuleParsingResult result = ruleItem.Rule.Accept(this);
				synthesizedCondition.Add(result.Condition, ruleItem.Connector);
				OnRuleProcessed(ruleItem.Rule, result);
			}

			return new RuleParsingResult(synthesizedCondition, string.Empty, string.Empty, string.Empty);
		}
	}
}
